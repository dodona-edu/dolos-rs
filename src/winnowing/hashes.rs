...
pub fn hash_token(token: &str) -> Hash {
    dbg!(token);
    token.bytes().fold(0, |hash, byte| ((hash + byte as usize) * BASE_TOKEN) % MOD)
}
...
mod tests {
    ...
    fn test_hash_tokens() {
        ...
        for i in 0..tokens.len() {
            assert_eq!(hash_token(&tokens[i]), hashes[i], "Mismatch for '{}'", tokens[i]);
        }
    }
    ...
    fn test_rolling_hash_k3() {
        ...
        let rolling_hashes: Vec<usize> = serde_any::from_file("fixtures/sample.rolling3.json").unwrap();
        let mut hasher = RollingHash::new(3);

        for i in 0..tokens.len() {
            assert_eq!(hasher.next_hash(token_hashes[i]), rolling_hashes[i], "Mismatch for '{}'", tokens[i]);
        }
    }
    ...
    fn test_rolling_hash_k17() {
        let tokens: Vec<String> = serde_any::from_file("fixtures/sample.tokens.json").unwrap();
        let token_hashes: Vec<usize> = serde_any::from_file("fixtures/sample.hashes.json").unwrap();
        let rolling_hashes: Vec<usize> = serde_any::from_file("fixtures/sample.rolling17.json").unwrap();
